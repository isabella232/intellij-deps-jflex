  ...
  private void emitCharMapArray() {
    ...
    println(
        "  public static int ZZ_CMAP(int ch) {\n"
            + "    return "
            + EmitterCM.genAccess(
                "ZZ_CMAP_" + "A", "ch", 16, sizes, (int[]) o[3], (int[]) o[4], (boolean[]) o[2])
            + ";\n"
            + "  }");
    println();
  ...
  private void emitGetRowMapNext() {
    ...
    println(
        "          int zzNext = zzTransL[ zzRowMapL[zzState] + "
            + (unpacked ? "zzCMapL[zzInput]" : "ZZ_CMAP(zzInput)")
            + " ];");
  ...
  private void emitActions() {
    ...
    for (Map.Entry<Action, Integer> entry : actionTable.entrySet()) {
      ...
      if (action.lookAhead() == Action.GENERAL_LOOK) {
        ...
        println(
            "                zzFState = zzTransL[ zzRowMapL[zzFState] + "
                + (unpacked ? "zzCMapL[zzInput]" : "ZZ_CMAP(zzInput)")
                + " ];");
        println("              }");
        println(
